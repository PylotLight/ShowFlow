...
test("QualityEngine Scoring", async () => {
  ...
  // Setup test data
  db.saveQuality({ id: 'q1', name: '720p', rank: 10 });
  db.saveQuality({ id: 'q2', name: '1080p', rank: 20 });
  db.saveQuality({ id: 'q3', name: '4K', rank: 30 });
  ...
  const file4K = "Show.S01E01.4K.mkv";
  ...
  const score4K = qualityEngine.getReleaseScore(file4K, 'p1');
  ...
  console.log(`Score 4K: ${score4K.totalScore}`);
  ...
  expect(score4K.totalScore).toBeGreaterThan(score1080HDR.totalScore);
