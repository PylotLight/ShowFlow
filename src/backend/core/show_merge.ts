...
/**
 * Detect shows that are likely the same underlying series stored twice.
 *
 *  - high: identical normalized canonical titles (the "-" vs ":" vs " "
 *    variants, "Re: ZERO" vs "Re-ZERO", case differences). These are the
 *    "safest matches" - punctuation/case variants of the same title almost
 *    certainly denote the same show.
 *  - fuzzy: titles that share tokens but aren't identical (e.g. one is a
 *    truncated/abbreviated version). These are surfaced for manual review
 *    only - merging them is risky.
 */
export function detectDuplicateShows(db: DatabaseManager): DuplicateGroup[] {
  ...
  const buildShowInfo = (show: any): DuplicateGroup["shows"][number] => {
    const episodes = db.listAllEpisodes(show.id);
    const files = db.listEpisodeFilesByShow(show.id).filter(f => f.is_current === 1);
    const rootFolderPath = db.getShowRootFolder(show.id);
    let folderName: string | null = null;
    if (rootFolderPath) {
      const names = new Set<string>();
      for (const ep of episodes) {
        if (!ep.file_path || !ep.file_path.startsWith(rootFolderPath)) continue;
        const rel = ep.file_path.slice(rootFolderPath.length).replace(/^[/\\]+/, "");
        const first = rel.split(/[/\\]/)[0];
        if (first) names.add(first);
      }
      folderName = names.size === 1 ? [...names][0]! : (names.size > 1 ? [...names].join(", ") : null);
    }
    return {
      id: show.id,
      title: show.title,
      year: show.year ?? null,
      seriesType: show.series_type ?? null,
      providerId: show.provider_id ?? null,
      providerType: show.provider_type ?? null,
  ...
  // High confidence: identical normalized title.
  for (const [norm, list] of byNorm) {
    if (list.length < 2) continue;
    if (consumed.has(norm)) continue;
    const showsInfo = list.map(r => buildShowInfo(r.show));
    consumed.add(norm);
    groups.push({
      key: norm,
      confidence: "high",
  ...
  // Fuzzy: token-overlap between distinct normalized titles.
  const keys = [...byNorm.keys()].filter(k => !consumed.has(k));
  for (let i = 0; i < keys.length; i++) {
    const keyA = keys[i];
    if (!keyA || consumed.has(keyA)) continue;
    const aTokens = new Set(keyA.split(" "));
    const matched: string[] = [keyA];
    for (let j = i + 1; j < keys.length; j++) {
      const keyB = keys[j];
      if (!keyB || consumed.has(keyB)) continue;
      const bTokens = new Set(keyB.split(" "));
      const score = jaccard(aTokens, bTokens);
      if (score >= 0.5) matched.push(keyB);
    }
    if (matched.length < 2) continue;
    for (const k of matched) consumed.add(k);
    const showsInfo = matched.flatMap(k => byNorm.get(k)!.map(r => buildShowInfo(r.show)));
    groups.push({
      key: keyA,
      confidence: "fuzzy",
