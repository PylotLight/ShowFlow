...
import { loadTheme, applyTheme, loadAccent, applyAccent } from "@frontend/lib/theme";
...
export function App() {
  ...
  const [exitingShow, setExitingShow] = React.useState<ShowSummary | null>(null);
  ...
  const showingShow = selected || exitingShow;
  ...
  return (
    <div className="app-background flex min-h-screen text-foreground pb-14 md:pb-0">
      ...
      {activeNav === "library" && !selected && backdropUrl && (
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        {showingShow ? (
          <div
            key={showingShow.id}
            className={`flex-1 flex flex-col min-w-0 transition-all duration-200 ${exitingShow ? "opacity-0 scale-[0.97]" : "animate-[slideUp_0.25s_ease-out]"}`}
          >
            <ShowDetail show={showingShow} onBack={handleBack} />
          </div>
        ) : (
          <>
